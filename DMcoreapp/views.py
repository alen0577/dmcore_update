...
def ex_base(request):
    ...
    usr = user_registration.objects.get(id=ids)
    today=date.today()
    count=daily_leeds_exists.objects.all().count()
    count=All_leads.objects.filter(date=today).annotate(count=Count('name'))
    print(count)
    context={
        "usr":usr,
        'count':count,
...
def dm_follow(request):
    ...
    count=All_leads.objects.filter(status__in=status_values).count()
    ...
    return render(request,'dm_manager/dm_follow.html',context)



def dm_followup(request):
    if request.session.has_key('userid'):
        ids = request.session['userid']
    else:
        return redirect('/')
    ...
    followup=All_leads.objects.filter(status__in=status_values).order_by('-followup_dt')
...
def get_cust_mail(request):
    ...
    item = All_leads.objects.filter(telecaller_id=cust, status="Follow up",assign_dt=date.today()).count()
...
# ----------------------------<digital marketing executive> <Alen Antony>------------    
